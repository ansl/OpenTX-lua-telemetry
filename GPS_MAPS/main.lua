...
local function circle_solid(xCenter, yCenter, radius,flags)
    local y, x
 ...
 local function circle(xCenter, yCenter, radius,width,flags)
  local y, x
  for y=-radius, radius do
    for x=-radius, radius do
      if(x*x+y*y <= radius*radius and x*x+y*y > (radius-width)*(radius-width))  then
      lcd.drawPoint(xCenter+x, yCenter+y,flags)
      end
    end
  end
...
local function GPS_DRONE_PLOT(wgt,x,y)
    if (wgt.DRONE.PCoor.curr.X~=0) then
      ...
      circle(wgt.DRONE.TPCoor.curr.X+x, wgt.DRONE.TPCoor.curr.Y+y, 8,2,CUSTOM_COLOR)
    end
 end
...
local function refresh(wgt)
  ...
  GPS_HOME_PLOT(wgt,1,1)
   ...
   lcd.drawText(wgt.zone.x+300, wgt.zone.y+110, wgt.DRONE.ARM,0)
   lcd.drawText(wgt.zone.x+300, wgt.zone.y+140, wgt.DRONE.FM,0)
