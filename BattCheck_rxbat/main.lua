...
local _options = {
  ...
  { "cellCount", VALUE , 4 }, -- default to '4'
  { "Color", COLOR, WHITE },
  { "Shadow", BOOL, 0 },
...
local function create(zone, options)
  ...
  if wgt.options.Sensor == 0 then
    wgt.options.Sensor = "RxBt"
  end
  if wgt.options.CelCount == 0 then
    wgt.options.CelCount = 4
  end
...
local function calculateBatteryData(wgt)
  ...
  if type(newCellData) ~= "number" then
    wgt.isDataAvailable = false
    return
  end


  wgt.cellSum = newCellData

  --- average of all cells
  wgt.cellAvg = wgt.cellSum / wgt.options.cellCount

  wgt.cellDataLive = newCellData

  -- mainValue
  if wgt.options.LowestCell == 1 then
    wgt.mainValue = wgt.cellAvg
  ...
  elseif wgt.options.LowestCell == 0 then
    wgt.secondaryValue = wgt.cellAvg
  else
  ...
  if (periodicHasPassed(wgt.periodic1) or wgt.cellPercent == 0) then
    ...
    wgt.cellPercent = getCellPercent(wgt, wgt.cellAvg) -- use batt percentage by lowest cell voltage
